...
func _init_game():
	bubble_1._bubble_twist(1,0.05)
	bubble_2._bubble_twist(1,0.05)
	bubble_3._bubble_twist(1,0.05)
	await get_tree().create_timer(BaseSetting.move_time).timeout
	bubble_1._bubble_appear("SocialBubble",bubble_end_location_1.position)
